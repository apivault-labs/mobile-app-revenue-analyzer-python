"""Thin synchronous client for the Mobile App Revenue & Competitor Analyzer Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, MobileAppRevenueAnalyzerError

ACTOR_ID = "apivault_labs~app-revenue-analyzer"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class MobileAppRevenueAnalyzerClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "mobile-app-revenue-analyzer-python/0.1.0",
        })
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise MobileAppRevenueAnalyzerError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise MobileAppRevenueAnalyzerError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise MobileAppRevenueAnalyzerError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
